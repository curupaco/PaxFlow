...
- **Arquitetura de Notificações Web Push Server-Side & Segurança VAPID [NEW]** — Sistema de envio de notificações Web Push totalmente server-side via Edge Function (`send-push`) no Supabase, com validação de tokens JWT/sessão de usuário e proteção por variáveis de ambiente privadas. Elimina chaves criptográficas privadas do bundle do cliente e gerencia inscrições com sincronização não intrusiva no boot apenas para permissões já concedidas.
- **Exclusão de Viagens em Cascata & Integridade Referencial [NEW]** — Motor de exclusão segura (`excluirViagemCascata`) que remove de forma ordenada e atômica todas as entidades filhas (comentários, notificações, recebimentos, conferências, lembretes, feedbacks NPS, reembolsos, tarefas, vínculos de passageiros e produtos), com captura de erros de foreign keys (`23503`) e feedback amigável ao gestor.
- **Fila de Notificações Visuais & Persistência Estrita de Alertas [NEW]** — Utilitário de toasts (`dialog.ts`) com gerenciamento seguro de timers ativos para evitar cancelamento prematuro em notificações concorrentes, e padronização Fail-Fast no `InboxService.archiveAlert` garantindo que alterações no Inbox só são refletidas após gravação real no Supabase.
...
- **Kanban de Viagens & Usabilidade Mobile [UPDATED]** — 5 colunas com drag-and-drop, SLAs visuais e gestão de produtos. **Redesenho 100% responsivo para smartphones com Botão Flutuante FAB (`+ Nova Viagem`), compartilhamento nativo de itinerários (`navigator.share`), modal full-screen para celular e gavetas inferiores (Bottom Sheets). Atualizado com Cópia Rápida de LOC em 1-Clique (`📋`) no cabeçalho geral, agrupamentos e produtos individuais, campo de busca em tempo real de alto desempenho, modal de gerenciamento ampliado com abas de Histórico de Reembolsos, aba de Produtos/Serviços, painel financeiro limpo e transparente com totalizadores consolidados em tempo real (**Valor da Viagem, Total em Produtos, Alocação e Rentabilidade Acumulada** sem sobreposições ou sugestões forçadas de sincronização), e persistência contínua de tela no salvamento dos detalhes da viagem. Conta com bloqueio inteligente de pagamento para produtos em edição no painel lateral (evitando recebimentos inconsistentes antes de salvar), além de inicialização consistente e íntegra de produtos dos tipos RAV e Markup. O código de reserva (LOC) do produto/serviço é obrigatório (máximo 20 caracteres, código único). Conta agora com a quitação financeira do LOC por meio de N Formas de Recebimento cadastradas (com suporte para remover todos os pagamentos e retornar ao estado de "Sem Pagamento"). Inclui o processo de Conferência Financeira individual por LOC (restrito a admin, com bloqueio rígido de edições e travamento caso haja pendência de recebimento) e o processo de Conferência de Processo por Viagem (bloqueando dados cadastrais cadastrados, com exceção de status/etapa editável, anexos e comentários funcionais). Apresenta **Novo Menu Popover de Conferência com Pills (`Conf.`)**: permite filtrar com precisão e seleção múltipla viagens com pendências (ex: `⏳ Fin. Pendente`, `✅ Fin. OK`, `⏳ Proc. Pendente`, `✅ Proc. OK`), atalhos rápidos (`✨ 100% OK` e `⏳ Nenhum OK`) e combinação lógica simultânea.**
- **Pipeline de Orçamentos** — 4 estágios com temperatura de lead, tags, notas e upload de documentos. **Atualizado com busca em tempo real e modal de visualização em duas colunas. A busca por clientes recorrentes no autocomplete do formulário foi corrigida para precisão absoluta. Ao fechar uma venda, caso o cliente não possua documento cadastrado, exige e valida o CPF/CNPJ com máscara e verificação oficial de dígitos verificadores (suportando a nova máscara CNPJ Alfanumérico da Receita Federal). A data de nascimento do passageiro e a Data Financeiro agora são obrigatórias na conversão, garantindo a consistência com o financeiro, enquanto a data de volta tornou-se opcional. Inclui agora o Check de Contato do Gestor ("Falamos c/ Cliente") exclusivo para Administradores nos cards de desistência, no modal e na tabela de relatórios, além da Reabertura de Orçamentos Desistidos (🔄) para mover o card de volta para 'Solicitado' com registro estruturado de motivo no histórico de negociações.**
...
- **Sistema de Gamificação dos Consultores [UPDATED]** — Engajamento operacional por meio de ganho de XP e patentes temáticas (Mochileiro, Explorador, Navegador, Guia de Elite, Embaixador). Apresenta anel de progresso circular SVG gradiente e nível numérico ao redor do avatar na Sidebar (sincronizados ao vivo via Supabase Realtime) e displays de patente sob o nome do usuário. As campanhas ativas permanecem sempre visíveis na barra lateral e no perfil durante toda a sua vigência, destacando o status de **Meta Atingida (100%)** com visual comemorativo dourado/esmeralda.
- **Mural de Medalhas (Badges) & Celebração Global Coletiva [UPDATED]** — Grade interativa no perfil com 14 medalhas conquistáveis (SLA_CHAMP, DRIVE_MASTER, COMPLIANCE_HERO, etc.) acompanhadas de tooltips flutuantes. **Celebração em Tempo Real na Agência**: quando qualquer consultor bate uma meta ou recebe uma medalha, o sistema dispara simultaneamente confetes, som triunfal sintetizado e modal celebrativo para todos os usuários conectados via Supabase Realtime e, para quem estava offline, exibe a conquista automaticamente no próximo login com persistência no banco de dados.
- **Animações e Efeitos de Celebração [UPDATED]** — Comemoração ao subir de nível ou fechar vendas com explosões visuais de confete (canvas-confetti via CDN) e áudios de chimes musicais sintetizados dinamicamente via Web Audio API, além de modal glassmorphic 3D personalizado com o nome do consultor destaque da agência.
