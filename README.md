...
- **Hub de Modelos de Mensagem (WhatsApp) [NEW]** — Módulo de gestão de templates nas Configurações (CRUD) e modal reativo de preview de disparos que substitui automaticamente dados do passageiro (`{{cliente}}`, `{{destino}}`, `{{link_itinerario}}` e `{{link_feedback}}`), adicionando botões e atalhos de envio rápido de WhatsApp com um clique no Dashboard, Ficha de Clientes e Orçamentos.
- **Campanhas de Vendas & Leaderboard [NEW]** — Aba unificada "Campanhas & Metas" no modal de perfil que lista campanhas ativas e renderiza o ranking (Leaderboard) de consultores ordenados por XP de forma a engajar a equipe.
