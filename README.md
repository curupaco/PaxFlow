...
- **Módulo de Cadastros (Gestão Dinâmica de Produtos e Destinos) [NEW]** — Nova página restrita a administradores que gerencia os tipos de produtos/serviços disponíveis na agência (permitindo customizar cores, ícones e campos extras), bem como a lista oficial de destinos nacionais e internacionais da agência (com suporte a autocomplete relacional, busca rápida por digitação, higienização de nomes e fallback para registros legados).
