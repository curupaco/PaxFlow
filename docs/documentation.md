...
- **Produtos, Detalhamento de Valores e Rentabilidade (Especialização por Tipo)**:
  ...
  - **Bloqueio de Pagamento para Produtos em Edição [NEW]**: Quando um produto estiver aberto no editor lateral (ou com alterações pendentes), o botão de pagamento do respectivo LOC é bloqueado e desabilitado visualmente (`opacity-55 cursor-not-allowed` e tooltip indicativa), impedindo o registro ou alteração de pagamentos antes que os dados do produto (especialmente de tipos como RAV e Markup) sejam salvos com sucesso no banco de dados.
