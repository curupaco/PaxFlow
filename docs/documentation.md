...
- **Painel de Filtros Avançados Combináveis & Resumo Financeiro em Tempo Real [UPDATED]**:
  ...
  - **Filtros de Markup & RAV por Produto**: Pills reativas de categorização (`Todos`, `💎⚡ Markup + RAV (e/ou)` [viagens com produtos de Markup ou RAV], `💎 Com Markup` [viagens com Markup > 0], `⚡ Com RAV` [viagens com RAV > 0], `✨ Ambos (MKP e RAV)` [viagens que possuem simultaneamente produtos com Markup e RAV], `💼 Apenas Comissão` [comissão pura sem markup nem rav] e `🚫 Sem MKP/RAV`).
...
- **Produtos, Detalhamento de Valores e Rentabilidade (Especialização por Tipo)**:
  - **Especialização por Tipo de Produto**:
    - **Produto "RAV - 12%" / "RAV"**: Apresenta apenas os campos **Venda (R$)** e **RAV (R$)**. A rentabilidade líquida é apurada automaticamente como `RAV * 0.88`, dispensando os campos de tarifa, taxa, comissão e markup.
    - **Produto "MARKUP"**: Apresenta apenas os campos **Venda (R$)** e **MARKUP (R$)**. A rentabilidade é apurada integralmente como `MARKUP`, dispensando taxas, comissão, rav e tarifa.
    - **Demais Produtos (Aéreo, Hotel, Seguro, etc.)**: Apresentam **Venda**, **Taxas (Embarque/Serviço)**, **Comissão** e **Tarifa Líquida** calculada (`Tarifa = Venda - Taxas - Comissão`). Os campos de Markup e RAV são permanentemente removidos.
  - Permite gerenciar itens de viagem preenchendo fornecedor, descrição, data do serviço, valor de venda, status e o **Código de Reserva (LOC)**.
  ...
  - **Bloqueio Inteligente de Pagamento para Produtos em Edição [UPDATED]**: Quando um produto estiver com alterações pendentes não salvas no editor lateral (`dirty`), o botão de pagamento do respectivo LOC é bloqueado e desabilitado visualmente (`opacity-55 cursor-not-allowed` e tooltip indicativa), impedindo o registro de pagamentos com dados obsoletos. Assim que o usuário clica em **Salvar Alterações** e o produto é gravado no banco com sucesso, o botão de pagamento do LOC é **imediatamente liberado** para recebimento e conferência, **sem a necessidade de fechar o painel lateral**.
  - **Inicialização Consistente de RAV e Markup [NEW]**: Ao adicionar produtos do tipo RAV ou Markup, o sistema inicializa os registros no banco com `rav = valor_venda` ou `markup = valor_venda` e `tarifa = 0`, prevenindo estados temporários inconsistentes entre a criação e o detalhamento.
  - **Detalhamento de Valores**: Após salvar o produto na viagem, ao clicar no item listado na aba "Produtos e Serviços", abre-se um modal de detalhamento que permite fracionar o valor de venda nas categorias: **Tarifa (Valor Líquido)**, **Taxa** e **Comissão** (ou **RAV** / **Markup** conforme a especialização do tipo de serviço).
...
- **Mural de Medalhas (Badges) e Celebrações Globais Coletivas**:
  ...
  - **Celebração em Tempo Real na Agência (Realtime Coletivo)**: Quando qualquer consultor bate uma meta ou recebe uma medalha, o evento é gravado na tabela `gamification_celebrations` e transmitido para todos os clientes online via Supabase Realtime, disparando confetes, som triunfal (Web Audio API) e o modal de conquista coletiva com o nome do consultor.
  - **Fila de Celebrações Pendentes (Zero-LocalStorage)**: Usuários que estavam offline no momento da conquista têm suas pendências recuperadas através do cruzamento com `gamification_celebration_views` nas últimas 48h, sendo exibidas no próximo login.
  - **Exibição Contínua de Campanhas Atingidas**: Campanhas cuja meta foi batida continuam visíveis na barra lateral e no perfil até o término da vigência (`data_fim >= hoje`), destacadas com o status `100% • Meta Atingida! 🏆` em tons esmeralda/dourado.
- **Sincronização em Tempo Real via Supabase Realtime**:
  - Inscrição reativa em tempo real (WebSockets) nas tabelas `profiles` e `gamification_celebrations`. Qualquer alteração no XP ou nova comemoração de meta atualiza instantaneamente a interface de todos os usuários conectados sem necessidade de recarregar a página.
...
- **Comemorações e Efeitos Visuais/Sonoros**:
  - **Level Up e Conquistas de Metas:** Dispara animações de confete (`canvas-confetti` carregado sob demanda via CDN) e sintetiza acordes musicais em tempo real usando a API nativa Web Audio API (sem carregar arquivos de áudio externos), abrindo também um modal glassmorphic premium em 3D.
...
- **Painel Geral de Relatórios (Abas Estratégicas de Auditoria)**:
  ...
  - **3. Faturamento e Rentabilidade [UPDATED]**: Exibe faturamento bruto, taxas, comissões de produtos, markup, RAV líquido (com retenção de 12%) e rentabilidade total real distribuídos por linha de serviço (aéreo, hospedagem, cruzeiro, seguro, etc.).
