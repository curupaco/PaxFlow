      ...
      `;
        ...
        listaEl.innerHTML = filtrados.map(c => {
            ...
            if (passSla.status === 'expired')
                borderSlaClass = 'border-l-4 border-l-rose-500 bg-rose-50/20';
            if (passSla.status === 'warning')
                borderSlaClass = 'border-l-4 border-l-amber-500 bg-amber-50/20';
        ...
        <button data-cliente-id="${c.id}" class="w-full text-left p-4 rounded-xl border ${isSelected
                ...
                : 'border-slate-100 hover:border-slate-200 bg-white hover:bg-slate-50/50 text-slate-700'} transition flex items-center justify-between gap-3 ${borderSlaClass} group">
