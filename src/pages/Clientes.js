...
export class ClientesPage {
     ...
     */
    checkPassaporteSLA(validadeStr) {
        if (!validadeStr)
            return { status: 'none', days: 0, message: 'Passaporte não cadastrado.' };
        const hoje = new Date();
    ...
    setupGlobalEventListeners() {
        ...
        searchInput?.addEventListener('input', (e) => {
            this.buscaTermo = e.target.value.toLowerCase();
     ...
     */
    filtrarERenderizarLista() {
        const listaEl = document.getElementById('lista-clientes-container');
        if (!listaEl)
            return;
        const filtrados = this.clientes.filter(c => c.nome.toLowerCase().includes(this.buscaTermo) ||
      ...
      `;
        ...
        listaEl.innerHTML = filtrados.map(c => {
            ...
            if (passSla.status === 'expired')
                borderSlaClass = 'border-l-4 border-l-rose-500 bg-rose-50/20 dark:bg-rose-950/10';
            if (passSla.status === 'warning')
                borderSlaClass = 'border-l-4 border-l-amber-500 bg-amber-50/20 dark:bg-amber-950/10';
            if (passSla.status === 'ok')
                borderSlaClass = 'border-l-4 border-l-emerald-500';
            return `
        <button data-cliente-id="${c.id}" class="w-full text-left p-4 rounded-xl border ${isSelected
                ...
                : 'border-slate-100 dark:border-slate-800/80 hover:border-slate-200 dark:hover:border-slate-750 bg-white dark:bg-slate-900 hover:bg-slate-50/50 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300'} transition flex items-center justify-between gap-3 ${borderSlaClass} group">
    ...
    setupFormEventListeners() {
        ...
        form?.addEventListener('submit', async (e) => {
            ...
            const documentoVal = document.getElementById('input-documento').value;
            const dataNascVal = document.getElementById('input-data-nasc').value;
            const enderecoVal = document.getElementById('input-endereco').value;
            const passNumeroVal = document.getElementById('input-pass-numero').value;
            const passValidadeVal = document.getElementById('input-pass-validade').value;
            const vistosVal = document.getElementById('textarea-vistos').value;
            const obsVal = document.getElementById('textarea-observacoes').value;
            const payload = {
                nome: nomeVal,
                email: emailVal,
        ...
        const fileInput = document.getElementById('file-input-documento');
    ...
    async handleFileSelected(file) {
        if (!this.clienteSelecionado || this.carregandoUpload)
            return;
        this.carregandoUpload = true;
        this.renderUploadState(true, file.name);
        try {
            const result = await uploadDocumentoCliente(this.clienteSelecionado.id, this.clienteSelecionado.nome, this.clienteSelecionado.email, this.clienteSelecionado.telefone, file);
    ...
    renderFichaDetalhada() {
      ...
      `;
        ...
        if (passSla.status === 'expired')
            passValidadeInputClass += ' passport-expired-alert text-rose-600 dark:text-rose-450';
        if (passSla.status === 'warning')
            passValidadeInputClass += ' passport-expired-alert text-amber-600 dark:text-amber-450';
        const isNew = !c.id;
        fichaEl.innerHTML = `
        ...
        <form id="form-cliente" class="space-y-6">
          ...
          <div class="border-t border-slate-100 dark:border-slate-800 pt-5">
            ...
            ${passSla.status !== 'none' && passSla.status !== 'ok' ? `
              <div class="mb-4 px-4 py-3 rounded-xl flex items-center gap-2 text-xs font-bold ${passSla.status === 'expired'
