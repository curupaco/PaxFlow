...
export class OrcamentosPage {
  ...
  private render(): void {
    ...
    const filtrados = this.orcamentos.filter(o => {
      ...
      return (
        ...
        (o.notasNegociacao && o.notasNegociacao.toLowerCase().includes(q)) ||
        donoNome.includes(q)
      );
    });
  ...
  private openVerNotasModal(id: string): void {
    ...
    const dono = this.consultores.find(c => c.id === orc.consultorId);
    ...
    const tempoAguardando = orc.createdAt ? this.calcularTempoAmigavel(orc.createdAt) : '';
          ...
          <div class="md:col-span-2 space-y-5">
            <div>
              ...
              <div class="p-3.5 bg-slate-50 dark:bg-slate-800/40 rounded-xl border border-slate-100 dark:border-slate-800 text-xs font-bold text-slate-700 dark:text-slate-350 space-y-1.5">
                ...
                <span class="block">Data da Viagem: <strong class="text-slate-800 dark:text-slate-100">${this.formatarDataBr(orc.dataViagem)}</strong></span>
          ...
          <div class="bg-slate-550/5 dark:bg-slate-800/20 p-4 rounded-2xl border border-slate-150 dark:border-slate-800 space-y-4 h-fit">
            ...
            <div>
              ...
              <div class="space-y-2">
                ...
                ` : ''}
                ${email ? `
                  <a href="${mailLink}" target="_blank" class="w-full px-3 py-2 bg-indigo-50/50 hover:bg-indigo-100 dark:bg-indigo-950/20 dark:hover:bg-indigo-900/30 text-indigo-650 dark:text-indigo-450 rounded-xl border border-indigo-100/30 dark:border-indigo-900/30 text-xs font-bold transition flex items-center gap-2 select-none">
