...
export class Dashboard {
  ...
  private renderTableRow(v: any): string {
    ...
    // Calcular Rentabilidade (Comissão + Markup) e Markup bruto
    let rentabilidade = 0;
    let totalMarkup = 0;
    if (v.produtos && Array.isArray(v.produtos)) {
      v.produtos.forEach((p: any) => {
        ...
        const rav = Number(p.rav) || 0;
        totalMarkup += markup;
        rentabilidade += comissao + markup + rav;
      });
    }
        ...
        <td class="px-5 py-4 whitespace-nowrap">
          ...
          ${totalMarkup > 0 ? `
            <div class="text-[9px] font-bold text-slate-500 dark:text-slate-400 mt-0.5 whitespace-nowrap">
              MKP: R$ ${totalMarkup.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
            </div>
          ` : ''}
        </td>
