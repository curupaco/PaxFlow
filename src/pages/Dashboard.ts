...
export class Dashboard {
  ...
  private render(): void {
    ...
    this.container.innerHTML = `
      ...
      <div class="dashboard-container bg-slate-50/50 dark:bg-slate-950 flex flex-col font-sans transition-colors duration-200 overflow-hidden">
        ...
        <header class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 md:sticky top-0 z-30 px-4 md:px-6 py-4 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 transition-colors duration-200 w-full max-w-full overflow-x-hidden">
  ...
  private async renderNextTripEngineWidget(): Promise<void> {
    ...
    try {
      ...
      if ((oportunidades.length === 0 && (window as any).paxflowForceNextTripDemo) || (window as any).paxflowSandbox) {
