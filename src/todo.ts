...
class TodoKanban {
    ...
    if (badge) {
      ...
      setTimeout(() => {
        badge.innerHTML = `
          ...
          <span class="text-slate-400 dark:text-slate-400 font-bold text-[10px] uppercase tracking-wider">Salvo</span>
        `;
      }, 1500);
    }
  }
    ...
    this.container.innerHTML = `
      ...
      <header class="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200/80 dark:border-slate-800/80 sticky top-0 z-30 px-6 py-4 flex flex-col md:flex-row items-center justify-between gap-4 transition-colors duration-200">
        ...
        <div class="flex flex-wrap items-center gap-3">
          <!-- Gerenciar Colunas -->
          <button id="btn-gerenciar-colunas" title="Configurar Colunas" class="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl transition border border-slate-200/60 dark:border-slate-800 flex items-center justify-center gap-1.5 font-bold text-xs">
            <svg width="16" height="16" class="w-4 h-4 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
              <path stroke-linecap="round" stroke-linejoin="round" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
              <path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            Colunas
          ...
          <!-- Alternar Tema -->
          <button id="todo-theme-toggle" title="Alternar Tema" class="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-400 rounded-xl transition border border-slate-200/40 dark:border-slate-700/40 flex items-center justify-center">
          ...
          <!-- Retornar ao CRM -->
          <a href="/" class="p-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-xl transition border border-slate-200/40 dark:border-slate-700/40 flex items-center justify-center gap-1.5 font-bold text-xs">
            <svg width="16" height="16" class="w-4 h-4 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
      ...
      <div class="px-6 py-4 bg-slate-100/50 dark:bg-slate-900/30 border-b border-slate-200/50 dark:border-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
        <div class="relative w-full sm:max-w-xs">
          <span class="absolute left-3 top-2.5 text-slate-400 flex items-center justify-center">
            <svg width="16" height="16" class="w-4 h-4 text-slate-400 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
      ...
      board.innerHTML = `
        <div class="flex-1 text-center py-20 animate-card-in">
          <div class="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-2xl flex items-center justify-center mx-auto mb-4 border border-slate-200 dark:border-slate-700/50">
            <svg class="w-8 h-8 text-slate-400 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5">
              <path stroke-linecap="round" stroke-linejoin="round" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
            </svg>
          </div>
        ...
        <div class="w-72 bg-slate-100/70 dark:bg-slate-900/40 border border-slate-200/50 dark:border-slate-800/80 rounded-2xl p-4 flex flex-col max-h-full min-h-[500px] shrink-0" data-column-id="${col.id}">
          ...
          <div class="flex items-center justify-between pb-3 mb-3 border-b border-slate-200/60 dark:border-slate-800/60">
            ...
            <div class="flex items-center gap-1.5 shrink-0">
              <!-- Adicionar rápido -->
              <button data-add-card-in-col="${col.id}" title="Adicionar cartão nesta coluna" class="p-1 hover:bg-slate-200 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 rounded-lg transition flex items-center justify-center">
                <svg width="12" height="12" class="w-3 h-3 text-slate-500 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2.5">
                  <path stroke-linecap="round" stroke-linejoin="round" d="M12 4.5v15m7.5-7.5h-15" />
                </svg>
              </button>
            </div>
          ...
          <div class="flex-1 flex flex-col gap-3 overflow-y-auto custom-scrollbar pr-1 select-none py-1" id="cards-container-${col.id}" data-col-target-id="${col.id}">
            ...
            ${colCards.length === 0 ? `
              <div class="flex-1 border-2 border-dashed border-slate-200 dark:border-slate-800/50 rounded-xl flex items-center justify-center p-6 text-center text-slate-400 dark:text-slate-400/80 text-[10px] font-bold uppercase tracking-wider select-none min-h-[80px]">
                Solte cartões aqui
              </div>
            ` : ''}
          </div>
  ...
  private renderCardHtml(c: TodoCard): string {
    ...
    const badgeColors: { [key: string]: string } = {
      low: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950/20 dark:text-emerald-400 border border-emerald-100/30 dark:border-emerald-900/10',
      ...
      high: 'bg-amber-50 text-amber-700 dark:bg-amber-950/20 dark:text-amber-400 border border-amber-100/30 dark:border-amber-900/10',
      urgent: 'bg-rose-50 text-rose-700 dark:bg-rose-950/20 dark:text-rose-400 border border-rose-100/30 dark:border-rose-900/10'
    };
      ...
      <div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800/85 p-4 rounded-xl shadow-sm hover:shadow-md hover:border-slate-300 dark:hover:border-slate-700/80 transition transform hover:-translate-y-0.5 group relative flex flex-col gap-3 select-none cursor-grab active:cursor-grabbing animate-card-in" data-card-id="${c.id}">
          ...
          <div class="flex items-center gap-1 select-none shrink-0">
            ${c.description ? `
              <button data-toggle-desc-id="${c.id}" class="p-1 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 rounded transition text-xs font-bold leading-none flex items-center justify-center" title="Expandir/Recolher descrição">
                <svg width="14" height="14" class="w-3.5 h-3.5 transform transition-transform duration-200 text-slate-400 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
            ...
            <div class="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-opacity duration-200 select-none">
              ...
              <button data-delete-card-id="${c.id}" class="p-1 hover:bg-rose-50 dark:hover:bg-rose-950/20 text-slate-300 hover:text-rose-600 dark:hover:text-rose-400 rounded transition" title="Excluir Cartão">
                <svg width="14" height="14" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        ${c.label ? `
          <div class="flex items-center">
            <span class="px-2.5 py-0.5 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 rounded-full font-black text-[9px] uppercase tracking-wide border border-slate-200/30 dark:border-slate-800/30">
              <svg width="10" height="10" class="w-2.5 h-2.5 inline mr-1 text-slate-400 dark:text-slate-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
        ...
        ${c.description ? `
          <p id="desc-${c.id}" class="hidden text-[11px] text-slate-400 dark:text-slate-400 font-medium leading-normal break-words pt-1.5 border-t border-slate-100 dark:border-slate-800/40 animate-card-in">
            ${c.description}
          </p>
        ` : ''}
        ...
        <div class="border-t border-slate-100 dark:border-slate-800 pt-2.5 flex items-center justify-between gap-2 mt-1 select-none">
          <div class="flex flex-col gap-1 overflow-hidden">
            <span class="block text-[8px] text-slate-400 dark:text-slate-400 font-bold uppercase tracking-wider leading-none">Prazo</span>
            <span class="text-[9px] text-slate-500 dark:text-slate-400 font-bold leading-none">${formatarDataBr(c.date) || 'Sem prazo'}</span>
          </div>
        ...
        : `<svg width="18" height="18" class="w-4.5 h-4.5 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
          ...
          <form id="form-todo-card" class="space-y-4">
            ...
            <div class="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input id="todo-card-date" type="date" value="${card?.date || ''}" class="w-full px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800 dark:text-slate-100 font-medium text-sm" />
              </div>
            ...
            <div>
              ...
              <select id="todo-card-column" required class="w-full px-3.5 py-2.5 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800 dark:text-slate-100 font-medium text-sm">
          ...
          : (defaultColId === col.id ? 'selected' : '')
            ...
            <div class="flex items-center justify-end gap-3 pt-3 border-t border-slate-100 dark:border-slate-800">
              <button id="btn-cancel-card-modal" type="button" class="px-5 py-2.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold text-xs tracking-wider rounded-xl transition uppercase">Cancelar</button>
      ...
      <div id="todo-cols-modal-overlay" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center opacity-0 transition-opacity duration-300">
        <div id="todo-cols-modal-card" class="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 text-slate-800 dark:text-slate-100 w-full max-w-md rounded-2xl shadow-2xl overflow-hidden transform scale-95 transition-all duration-300 p-6 flex flex-col gap-4">
          ...
          <div class="space-y-4 max-h-[300px] overflow-y-auto custom-scrollbar pr-1" id="manage-cols-list-container">
            ${this.columns.map((col, idx) => `
              <div class="flex items-center gap-3 p-2.5 border border-slate-200/60 dark:border-slate-800 rounded-xl bg-slate-50 dark:bg-slate-950/50 hover:bg-slate-100/50 dark:hover:bg-slate-950/80 transition">
                ...
                <button data-delete-col-id="${col.id}" class="p-2 hover:bg-rose-50 dark:hover:bg-rose-900/20 text-slate-500 dark:text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 rounded-lg transition animate-card-in" title="Remover Coluna">
                  <svg width="14" height="14" class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                  </svg>
                </button>
          ...
          <div class="pt-2 border-t border-slate-100 dark:border-slate-800">
            <label class="block text-[10px] font-bold text-slate-400 dark:text-slate-400 uppercase mb-1">Adicionar Nova Coluna</label>
