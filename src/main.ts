...
class App {
    ...
    try {
      ...
      if (error || !user) {
        ...
        if (currentHash && currentHash.length > 1) {
          ...
          if (pagePart && ['analytics', 'inbox', 'orcamentos', 'dashboard', 'reembolsos', 'conciliacao', 'clientes', 'next-trip', 'relatorios', 'cadastros', 'configuracoes'].includes(pagePart)) {
        ...
        window.addEventListener('hashchange', () => {
          ...
          if (h && h.length > 1) {
            ...
            if (pPart && ['analytics', 'inbox', 'orcamentos', 'dashboard', 'reembolsos', 'conciliacao', 'clientes', 'next-trip', 'relatorios', 'cadastros', 'configuracoes'].includes(pPart)) {
    ...
    this.container.innerHTML = `
      ...
      <div class="min-h-screen w-full max-w-full overflow-x-hidden flex flex-col md:flex-row bg-slate-50/50 dark:bg-slate-950 transition-colors duration-200">
        ...
        <aside id="app-sidebar" class="fixed inset-y-0 left-0 h-screen w-64 md:${this.sidebarCollapsed ? 'w-20' : 'w-64'} bg-white dark:bg-slate-900 text-slate-600 dark:text-slate-300 flex flex-col border-r border-slate-200 dark:border-slate-800/60 shadow-2xl md:shadow-xl z-50 md:z-50 transition-all duration-300 transform -translate-x-full md:translate-x-0 pt-safe pb-safe">
          ...
          <!-- Links de Navegação -->
          <nav class="flex-1 ${this.sidebarCollapsed ? 'px-2 py-2' : 'py-2 px-3'} space-y-0.5 flex flex-col justify-between overflow-y-auto overflow-x-hidden custom-scrollbar">
            <div class="space-y-0.5">
              ...
              <!-- Link: Conciliação Bancária (Admin / Gestão) -->
              ${(() => {
                if (this.perfil?.role === 'admin') {
                  return `
                    <button id="nav-conciliacao" class="w-full px-3 py-1.5 rounded-xl flex items-center ${this.sidebarCollapsed ? 'justify-center' : 'justify-start'} gap-2.5 font-semibold text-xs text-left transition select-none group">
                      <svg width="18" height="18" class="w-4.5 h-4.5 text-slate-400 group-hover:text-slate-600 dark:text-slate-400 dark:group-hover:text-slate-300 group-[.bg-indigo-600]:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                      </svg>
                      <span class="${this.sidebarCollapsed ? 'md:hidden' : ''}">Conciliação</span>
                    </button>
                  `;
                }
                return '';
    ...
    if (footerContainer) {
      footerContainer.innerHTML = `
        <div class="flex flex-col gap-2 border-t border-slate-100 dark:border-slate-800 pt-3 w-full">
          ...
          <div class="flex items-center justify-center gap-1.5 w-full ${this.sidebarCollapsed ? 'flex-col mt-1 px-0' : 'flex-row px-1'}">
            <!-- Theme Toggle -->
            <button id="theme-toggle-btn" title="Alternar Tema" class="${this.sidebarCollapsed ? 'w-9 h-9 p-0 justify-center' : 'flex-1 py-1.5 px-2 justify-center gap-1.5'} bg-slate-50 hover:bg-slate-100 dark:bg-slate-800/60 dark:hover:bg-slate-800 text-slate-600 hover:text-slate-900 dark:text-slate-300 dark:hover:text-white rounded-xl transition border border-slate-200/50 dark:border-slate-700/50 flex items-center shrink-0 text-xs font-semibold select-none">
              <svg width="16" height="16" class="w-4 h-4 theme-icon-light shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
              </svg>
              <svg width="16" height="16" class="w-4 h-4 theme-icon-dark shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364-6.364l-.707.707M6.343 17.657l-.707.707m12.728 0l-.707-.707M6.343 6.364l-.707-.707M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
              </svg>
              <span class="${this.sidebarCollapsed ? 'hidden' : ''}">Tema</span>
            </button>
            <!-- Logout -->
            <button id="sidebar-logout-btn" title="Sair do Sistema" class="${this.sidebarCollapsed ? 'w-9 h-9 p-0 justify-center' : 'flex-1 py-1.5 px-2 justify-center gap-1.5'} bg-slate-50 hover:bg-rose-50 dark:bg-slate-800/60 dark:hover:bg-rose-950/30 text-slate-600 hover:text-rose-600 dark:text-slate-300 dark:hover:text-rose-400 rounded-xl transition border border-slate-200/50 dark:border-slate-700/50 flex items-center shrink-0 text-xs font-semibold select-none">
              <svg width="16" height="16" class="w-4 h-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
              </svg>
              <span class="${this.sidebarCollapsed ? 'hidden' : ''}">Sair</span>
            </button>
          </div>
        </div>
      `;
  ...
  private setupNavigationListeners(): void {
    const pages = ['analytics', 'inbox', 'orcamentos', 'studio', 'dashboard', 'reembolsos', 'conciliacao', 'clientes', 'next-trip', 'relatorios', 'cadastros', 'configuracoes'];
  ...
  private navigate(page: string, extraId?: string): void {
    ...
    const navButtons = ['analytics', 'inbox', 'orcamentos', 'studio', 'dashboard', 'reembolsos', 'conciliacao', 'clientes', 'next-trip', 'relatorios', 'cadastros', 'configuracoes', 'ajuda'];
